      ...
      <div className="relative h-[600px] overflow-hidden">
        {banners.map((banner, index) => (
          <div key={index} className={`absolute inset-0 transition-all duration-1000 ease-in-out`} style={{ transform: `translateX(${(index - currentSlide) * 100}%)`, opacity: index === currentSlide ? 1 : 0 }}>
            <div className={`h-full bg-gradient-to-r ${banner.bg} flex items-center justify-center text-white relative overflow-hidden`} style={{ transform: `translateY(${scrollY * 0.3}px)` }}>
              <div className="absolute inset-0 overflow-hidden">
                {[...Array(20)].map((_, i) => (
                  <div key={i} className="absolute text-4xl animate-float" style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, animationDelay: `${Math.random() * 5}s`, animationDuration: `${5 + Math.random() * 10}s` }}>
                    {banner.particles}
                  </div>
                ))}
              </div>
              <div className="text-center px-6 relative z-10">
                ...
                <h1 className="text-6xl md:text-7xl font-black mb-6 drop-shadow-2xl animate-fade-in-up">{banner.title}</h1>
                <p className="text-3xl mb-8 opacity-90 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>{banner.subtitle}</p>
                <Link to="/" className="inline-block bg-white text-black px-10 py-5 rounded-full font-black text-xl hover:bg-yellow-400 transition-all duration-300 transform hover:scale-110 shadow-2xl animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                  Shop Now →
                </Link>
              </div>
            </div>
        ...
        <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-4 z-10">
          {banners.map((_, index) => (
            <button key={index} onClick={() => setCurrentSlide(index)} className={`h-3 rounded-full transition-all duration-300 ${index === currentSlide ? "bg-white w-12" : "bg-white bg-opacity-50 w-3"}`} />
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-16">
          {[
            ...
            <div key={idx} className={`rounded-2xl p-8 text-center shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:rotate-1 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <div className={`text-5xl mb-4 inline-block p-4 rounded-full bg-gradient-to-r ${stat.color}`}>{stat.icon}</div>
              <h3 className={`font-bold text-xl mb-2 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{stat.label}</h3>
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>{stat.value}</p>
            </div>
          ))}
        </div>
        ...
        <section className={`mt-20 rounded-3xl p-16 text-center shadow-2xl backdrop-blur-xl ${darkMode ? 'bg-gradient-to-r from-purple-900/50 to-pink-900/50 border border-purple-500/30' : 'bg-gradient-to-r from-purple-600 to-blue-600'} text-white relative overflow-hidden`}>
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row max-w-xl mx-auto gap-4">
              <input type="email" placeholder="Enter your email" className={`flex-1 px-8 py-5 rounded-full focus:outline-none focus:ring-4 focus:ring-yellow-400 text-lg ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`} />
              <button className="bg-gradient-to-r from-yellow-400 to-orange-400 text-black px-10 py-5 rounded-full font-black text-lg hover:from-yellow-300 hover:to-orange-300 transition-all duration-300 transform hover:scale-105 shadow-xl whitespace-nowrap">Subscribe</button>
            </div>
          </div>
        </section>
      </div>
